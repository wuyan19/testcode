...
uint32_t crc32_no_comp(uint32_t crc, const char *buf, uint32_t len);

unsigned int compute(int argc, char* argv[])
{
	int fd_in;
	...
	{
		read(fd_in, p, PAGE_LEN);
		p += PAGE_LEN;
		
	}
	read(fd_in, p, size%PAGE_LEN);

	result_crc = crc32(0, (unsigned char *)data, size);
